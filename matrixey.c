...
int startagain() {
	...
	scanf(" %c%*[^\n]", &restart);
	/* Space before % is so it ignores whitespace chars because that's not 
	typical for %c unlike other format types %*[^\n] is supposed to catch the 
	extra input and discard it(Though in other situations I have had it not 
	work, it does in this one */
	if (restart == 'y')
		return 0;
	else
		return 1;
